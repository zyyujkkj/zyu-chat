...
{
    ui->setupUi(this);
    ui->pass_edit->setEchoMode(QLineEdit::Password);//设置成密码模式
    ui->confirm_edit->setEchoMode(QLineEdit::Password);
    ui->err_tip->setProperty("state", "normal");//相当于告诉qt我们有一个正常的状态和不正常的状态
    repolish(ui->err_tip);
    ...
    ui->pass_visible->setCursor(Qt::PointingHandCursor);//鼠标滑上去变小手
...
void RegisterDialog::on_get_code_clicked()//点击后有个回调
{
    auto email = ui->email_edit->text();//获取email这里的文本
    ...
    if(match){
        ...
        showTip(tr("邮箱地址不正确"), false);//需要封装一个showTip函数来做提示
    }
}
...
{
    _tip_errs[te] = tips;
    showTip(tips, false);//把错误类型和提示的话放入_tip_err这个map里，并作一次展示
}
...
bool RegisterDialog::checkUserValid()//若输入用户为空就加错误提示，不为空就清除错误
{
...
void RegisterDialog::on_sure_btn_clicked()
{
    ...
    HttpMgr::GetInstance()->PostHttpReq(QUrl(gate_url_prefix+"/user_register"),//发送请求user_register
