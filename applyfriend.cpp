...
void ApplyFriend::addLabel(QString name)
{
    if (_friend_labels.find(name) != _friend_labels.end()) {
        ui->lb_ed->clear();
        return;
    }//判断friend_label里有没有这个名字，如果有就不添加
    //如果没有，就创建FriendLabel并添加
    ...
    connect(tmplabel, &FriendLabel::sig_close, this, &ApplyFriend::SlotRemoveFriendLabel);//点击关闭后移出这个friendlabel
    ...
    if (ui->gridWidget->height() < _label_point.y() + tmplabel->height() + 2) {
        ui->gridWidget->setFixedHeight(_label_point.y() + tmplabel->height() * 2 + 2);//超过白框高度的话就要重新设置白框高度
    }
}
